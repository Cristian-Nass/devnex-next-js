...
export class AuthController {
  ...
  async login(
    ...
    // Mirror the JWT into a cross-subdomain cookie so sibling apps
    // (builder.netmart.se, ...) get the same session for free.
    setSessionCookie(res, this.config, result.access_token);
    return result;
  }
  ...
  // ─── Introspect (cross-subdomain identity for sibling apps) ──────────────
  //
  // Sibling apps (builder.netmart.se, wp.netmart.se, ...) call this from
  // their server-side code, forwarding the user's cookie or Bearer token.
  // The global JwtAuthGuard validates either credential via JwtStrategy and
  // populates `req.user`; we just return the safe-user shape.
  //
  // Defined on both GET and POST so consumers can choose: GET is the
  // canonical idempotent shape, POST helps with strict CORS preflight rules.

  @Get('introspect')
  introspectGet(@CurrentUser() user: { id: string }) {
    return this.authService.getMe(user.id);
  }

  @Post('introspect')
  @HttpCode(HttpStatus.OK)
  introspectPost(@CurrentUser() user: { id: string }) {
    return this.authService.getMe(user.id);
  }
  ...
  async googleCallback(@Req() req: Request, @Res() res: Response) {
    ...
    // Set the cross-subdomain cookie *before* the redirect so the user lands
    // on the frontend already authenticated for every sibling subdomain.
