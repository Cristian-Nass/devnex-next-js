...
const WebsiteProductsPage = async () => {
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8">
      <h1
        className={cn(
          'text-4xl font-bold text-center mb-4 pt-28',
          ubuntu.className,
        )}>
        {t('title')}
      </h1>
      <p className={cn('text-lg text-center mb-4 pb-12', ubuntu.className)}>
        {t('subtitle')}
      </p>
      <div className="flex flex-row gap-6 w-full">
        <div className="w-1/4 text-center p-4 rounded-lg border-1 border-cyan-600 flex flex-col justify-between">
          <h2 className={cn('text-2xl font-bold mb-4', ubuntu.className)}>
            {t('plans.launch.name')}
          </h2>
          <ul
            className={cn(
              'list-none text-left text-gray-800 space-y-1.5 h-full',
              ubuntu.className,
            )}>
            {[
              'customizable',
          ...
          <Button
            asChild
            className={cn('w-1/2 h-10 mx-auto mt-4', ubuntu.className)}>
        ...
        <div className="w-1/4 text-center p-4 rounded-lg border-1 border-cyan-600 flex flex-col justify-between">
          ...
          <ul
            ...
            <li className="flex items-center gap-2">
              <CheckIcon className="w-4 h-4" />
              {t('features.webApplicationMobile')}
            </li>
