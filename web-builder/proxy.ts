...
/**
 * Paths that never require authentication. The locale prefix is added back
 * by next-intl before this list is consulted (we strip it before checking).
 */
const PUBLIC_PREFIXES = ["/api/sites/public", "/_next", "/favicon", "/robots", "/sitemap"];
...
function isPublicPath(pathname: string): boolean {
  ...
  return PUBLIC_PREFIXES.some((p) => bare === p || bare.startsWith(p + "/") || bare.startsWith(p));
}

function buildLoginRedirect(req: NextRequest, locale: string): NextResponse {
  // Use the request URL (post-proxy) so the user lands back on whatever
  // page they tried to open after signing in on the platform.
 ...
 *   2. We gate non-public routes on the cross-subdomain `netmart_session`
...
export function proxy(request: NextRequest): NextResponse {
  ...
  // next-intl issued its own redirect (e.g. `/` → `/en`). Honour it.
